...
def home(request):
    return render (request, 'idealer/home.html')

def main(request):
    context = {
        'inventory' : Add.objects.all()
    }
    return render (request, 'idealer/main.html', context)
...
def contact(request):
    ...
    return render(request, 'idealer/contact.html')

def registration(request):
    return render (request, 'idealer/registration.html')
...
def create_add(request):
    if request.method == 'POST':
        ...
        new_post = Add.objects.create(make=request.POST['make'], model=request.POST['model'], year=request.POST['year'], color=request.POST['color'], creator = User.objects.get(id=request.session['user_id']))
        print(new_post)
